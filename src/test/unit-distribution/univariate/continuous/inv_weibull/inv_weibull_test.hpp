...
class AgradDistributionsInvWeibull : public AgradDistributionTest {
public:
  ...
  log_prob(const T_y& y, const T_shape& alpha, const T_scale& sigma,
    ...
    return stan::prob::inv_weibull_log(y, alpha, sigma);
  }
  ...
  log_prob(const T_y& y, const T_shape& alpha, const T_scale& sigma,
     const T3&, const T4&, const T5&, const T6&, const T7&, const T8&, const T9&) {
    return stan::prob::inv_weibull_log<propto>(y, alpha, sigma);
  }
...
TEST(ProbDistributionsWeibull,Cumulative) {
  using stan::prob::inv_weibull_cdf;
  using std::numeric_limits;
  EXPECT_FLOAT_EQ(0.60653065, inv_weibull_cdf(2.0,1.0,1.0));
  EXPECT_FLOAT_EQ(2.74433842e-5, inv_weibull_cdf(0.8,2.9,1.8));
  EXPECT_FLOAT_EQ(0.0, inv_weibull_cdf(0.25,3.9,1.7));

  // EXPECT_FLOAT_EQ(0.0,
  //                 weibull_cdf(-numeric_limits<double>::infinity(),
  //                             1.0,1.0));
  EXPECT_FLOAT_EQ(0.0, inv_weibull_cdf(0.0,1.0,1.0));
  EXPECT_FLOAT_EQ(1.0, inv_weibull_cdf(numeric_limits<double>::infinity(),
                                   1.0,1.0));
}
